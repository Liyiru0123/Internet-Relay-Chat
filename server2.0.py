    ...
    try:
        ...
        while not user.is_quit:
            try:
                ...
                print(f"User {user.nick or user.addr} timed out")
                break
            ...
            while b"\r\n" in user.buffer:
                ...
                if line:
                    print(f"<< {line}")  # Log incoming message
        ...
        print(f"Disconnected from: {user.addr} (leaving)")
...
def send_welcome_messages(user: User):
    ...
    user.send(f":server 002 {user.nick} :Running version 1.0")
    user.send(f":server 003 {user.nick} :This server was created for assignment1")
    user.send(f":server 004 {user.nick} :server 1.0")
...
def process_command(user: User, line: str):
    ...
    # set username and realname
        ...
        if len(parts) >= 4:
            ...
            # 处理 realname：有 : 则取后面所有内容，无则拼接 parts[4:]（兼容短输入）
            if ":" in line:
                user.realname = line.split(":", 1)[1]
            else:
                user.realname = " ".join(parts[4:]) if len(parts) >=5 else user.user
